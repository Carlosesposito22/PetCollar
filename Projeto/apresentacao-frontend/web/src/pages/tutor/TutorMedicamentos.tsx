  ...
  return (
    <div className="space-y-6">
      ...
      <div className="card p-6 space-y-5">
        <header>
          <p className="text-sm text-ink-500">Emitida em {dataAss}</p>
          <p className="text-sm text-ink-500">
            Tratamento de {new Date(prescricao.dataInicio).toLocaleDateString("pt-BR")} até{" "}
          ...
          <div className="space-y-3">
            {prescricao.itens.map((it, i) => (
              <div key={i} className="rounded-xl border-2 border-brand-200 bg-brand-50/30 p-4">
                <p className="text-base font-semibold text-brand-800">{it.nomeMedicamento}</p>
                <dl className="mt-3 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-3">
                  <Linha rotulo="Dose total" valor={`${fmt(it.doseTotalMg, 2)} mg`} destaque />
                  ...
                  <Linha rotulo="Horários" valor={it.horarios.join(", ") || "—"} />
                </dl>
                {it.notaCuidado && (
        ...
        {prescricao.instrucoesGerais.length > 0 && (
          <section>
            <h2 className="mb-2 text-base font-semibold text-ink-900">Instruções gerais</h2>
